...
class CameraCalibrator:
    ...
    def genCornerCoordinates(self, u_meas, v_meas):
        ...
        for i in range(len(u_meas)):
            Xg.append(u_meas[i] - u_meas[i][0])
            Yg.append(v_meas[i] - v_meas[i][0])
    ...
    def estimateHomography(self, u_meas, v_meas, X, Y):  # Zhang Appendix A
        ...
        """
        ########## Code starts here ##########
        M_tilde = np.array([X, Y, np.ones(X.shape[0])]).T       
        row1 = np.hstack((M_tilde, np.zeros((X.shape[0],3)),  -np.multiply(np.tile(u_meas,(3,1)).T,M_tilde)))
        row2 = np.hstack((M_tilde, np.zeros((X.shape[0],3)),  -np.multiply(np.tile(v_meas,(3,1)).T,M_tilde)))
